...
impl<'a> HostSession<'a> {
    ...
    /// Commit a freshly-built entity into the document, returning its handle.
    pub(crate) fn commit_entity_handle(&mut self, entity: EntityType) -> Option<Handle> {
        self.app.commit_entity_handle(entity)
    }

    /// DocApi `push_undo`: begin an entity-only delta capture for one op.
    pub(crate) fn begin_doc_api_undo(&mut self, label: &str) {
        self.doc_api_pending = self.app.begin_undo(self.tab, label, 1, true);
    }
