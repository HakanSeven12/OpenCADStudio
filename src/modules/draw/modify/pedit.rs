...
mod editing_tests {
    ...
    #[test]
    fn multiple_decline_and_zero_fuzz_join_keep_the_command_active() {
        ...
        assert!(matches!(joined.on_text_input("0.25"), Some(CmdResult::ReportError(_))));
        let Some(CmdResult::PeditOp { op: PeditOp::JoinSelection(selected), .. }) =
            joined.on_text_input("0")
        else {
