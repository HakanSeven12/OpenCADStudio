...
pub(super) fn tool_icon(tool: &ToolDef) -> IconKind {
    crate::ui::command_presentation::ribbon_command_icon(tool_command(tool), tool.icon)
}
    ...
    match item {
        ...
        | RibbonItem::LargeDropdown {
            ...
            let cur_label = last_cmd
                ...
                .unwrap_or(*id);
            let cur_icon = crate::ui::command_presentation::ribbon_menu_icon(last, cur_icon);
...
pub(super) fn quick_access_btn<'a>(
    ...
    let icon = make_icon(
        crate::ui::command_presentation::ribbon_command_icon(cmd, IconKind::Svg(icon_bytes)),
        16.0,
    );
    let btn = button(
        container(icon)
